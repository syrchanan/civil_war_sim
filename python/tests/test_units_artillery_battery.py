...
def test_artillery_unit_type():
    bat = ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=8, subtype='battery')
...
def test_artillery_subtype_valid():
    for subtype in UNIT_SUBTYPES['art']:
        bat = ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=8, subtype=subtype)
        assert bat.subtype == subtype

def test_artillery_subtype_in_str():
    bat = ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=8, subtype='siege battery')
    assert 'siege battery' in str(bat)

def test_artillery_invalid_subtype_raises():
    with pytest.raises(ValueError, match="subtype"):
        ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=8, subtype='dragoons')

def test_artillery_missing_subtype_raises():
    with pytest.raises(TypeError):
        ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=8)
...
def test_effective_guns_over_crewed():
    ...
    bat = ArtilleryBattery(size=100, stats='4/4/0/0', law='ln', guns=8, subtype='battery')
    assert bat.effective_guns == 8

def test_effective_guns_crew_limited():
    # 6 guns * 7 = 42 crew needed, only 35 (5 guns worth)
    bat = ArtilleryBattery(size=35, stats='4/4/0/0', law='ln', guns=6, subtype='battery')
    assert bat.effective_guns == 5

def test_effective_guns_zero_crew():
    bat = ArtilleryBattery(size=0, stats='4/4/0/0', law='ln', guns=8, subtype='battery')
    assert bat.effective_guns == 0

def test_effective_guns_zero_guns():
    bat = ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=0, subtype='battery')
    assert bat.effective_guns == 0
...
def test_artillery_invalid_law_raises():
    with pytest.raises(ValueError):
        ArtilleryBattery(size=56, stats='4/4/0/0', law='xx', guns=8, subtype='battery')

def test_artillery_invalid_stats_raises():
    with pytest.raises(ValueError):
        ArtilleryBattery(size=56, stats='bad/stats', law='ln', guns=8, subtype='battery')

def test_artillery_negative_guns_raises():
    with pytest.raises(ValueError, match="guns"):
        ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=-1, subtype='battery')

def test_artillery_non_int_guns_raises():
    with pytest.raises(TypeError, match="guns"):
        ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=4.0, subtype='battery')
...
def test_artillery_position_at_construction():
    ...
    bat = ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=8, subtype='battery', position=pos)
    assert bat.position == pos

def test_artillery_flat_distance_to():
    a = ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=8, subtype='battery')
    b = ArtilleryBattery(size=56, stats='4/4/0/0', law='ln', guns=8, subtype='battery')
